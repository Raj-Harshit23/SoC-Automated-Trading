...
def w_closed_form(X, Y):
    ...
    Xt=torch.transpose(X,0,1).double()
    w_closed=torch.mm(torch.mm(torch.linalg.inv(torch.mm(Xt,X.double())),Xt),Y)
...
def transform_features(X, degree):
    ...
    if X.shape[1]==1:
        phi_X=torch.zeros(X.shape[0],degree+1).double()
        for i,row in enumerate(X):
            for h in range(degree+1):
                phi_X[i][h]=torch.pow(row,h).double()
        # print(phi_X)
    else:
        phi_X=torch.zeros(X.shape[0],degree+1).double()
        # print(X.shape)
        ...
        for i,row in enumerate(X):
            for h in range(degree+1):
                phi_X[i][h]=torch.linalg.norm(row,ord=h).double()
        # print(phi_X)
        # Student code end
    # print(phi_X)
    return phi_X
...
def l2_loss(X, Y, w):
    ...
    # loss = YOUR CODE HERE
    loss=torch.sum(((Y-torch.mm(X.double(),w))**2),dim=0)/(X.shape[0])
...
def l2_loss_derivative(X, Y, w):
    ...
    derivative=(2*torch.mm(torch.mm(torch.transpose(X,0,1),X),w)-2*torch.mm(torch.transpose(X,0,1),Y))/(X.shape[0])
...
def train_model(X_train, Y_train, X_test, Y_test, w, eta):
    ...
    test_err+=new_loss
    while(abs(new_loss-old_loss)>epsilon):
        old_loss=new_loss
...
if __name__ == '__main__':
    ...
    for degree in possible_degrees:
        ...
        result.append([degree, l2_loss_train, l2_test_loss])
    result = np.array(result)
    np.savetxt('output.txt', result, fmt="%f")
    check=np.sum(result,axis=1)
    # print(check)
    optimal_degree=np.argmin(check)+1
    # print("optimal degree", optimal_degree)
    with open('output.txt', 'a') as f:
        f.write(f"optimal degree {optimal_degree}")
